...
int Alarm = 0;  // Global variable to indicate an alarm
// TIME VARIABLES:
    ...
    if (ProwlCodeIndex != -1) {
      ...
      if (ProwlCodeEnd != -1) {
        ...
        if (ProwlReturnCode == "200") {                                                  // Check the success code and print the appropriate message
          Serial.println("[+] Send Push message " + description);
        } else {
          Serial.println("[-] Failed to send Push message " + description);
        }
      } else {
...
void connectToWiFi() {
  ...
  //wifiManager.setTimeout(3600); // Sets timeout until configuration portal gets turned off
...
void timerCallback(void *pArg) {
  ...
  if (inp > 0) {
    int Diff = millis() - lastTriggerTime;
    if (Diff >= skipDuration) {  // Check if the skip duration has elapsed
      // Serial.println("[#] Input trigger HIGH");      
      // Serial.println("[#] Milliseconds: " + String(millis()) + " lastTriggerTime is: " + lastTriggerTime + " skipDuration is: " + skipDuration + " Diff is: " + Diff);
      ...
      Serial.println("[+] Ignoring alarms for the next 1 minute");
       ...
       //   Serial.println("[#] Input trigger HIGH (Ignored during grace period)");
       //
       // }
  } else {
    // Serial.println("[#] Input trigger LOW");
